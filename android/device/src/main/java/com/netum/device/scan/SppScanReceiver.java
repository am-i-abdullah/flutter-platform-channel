...
public abstract class SppScanReceiver extends BroadcastReceiver {
    ...
    private static final class ScanHandler extends Handler {
        ...
        private final WeakReference<SppScanReceiver> mSppScanReceiver;

        ScanHandler(Looper looper, SppScanReceiver sppScanReceiver) {
            super(looper);
            mSppScanReceiver = new WeakReference<>(sppScanReceiver);
        }

        @Override
        public void handleMessage(Message msg) {
            SppScanReceiver sppScanReceiver = mSppScanReceiver.get();
            if (sppScanReceiver != null) {
                if (msg.what == BleMsg.MSG_SCAN_DEVICE) {
                    final BleDevice bleDevice = (BleDevice) msg.obj;
                    if (bleDevice != null) {
                        sppScanReceiver.handleResult(bleDevice);
                    }
                }
            }
        }
    ...
    public void prepare(String[] names, String mac, boolean fuzzy, boolean filter, boolean needConnect,
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BluetoothDevice.ACTION_FOUND)) {
            ...
            BleLog.i("->onBluetoothFound() - name=" + device.getName() + " , address=" + device.getAddress());
            onLeScan(device, 0, null);
            //发现新设备（SPP）
        }
        if (action.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED)) {
            BleLog.i("->onBluetoothDiscoveryFinished()");
            //设备扫描完毕（SPP）
            //if (listener != null) {
        ...
        if (action.equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)) {
            BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
            BleLog.i("->onBluetoothBondStateChanged() - name=" + device.getName() + " , address=" + device.getAddress());
            //设备绑定状态（SPP）
            //if (listener != null) {
        ...
        if (action.equals(BluetoothDevice.ACTION_ACL_CONNECTED)) {
            BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
            BleLog.i("->onBluetoothConnected() - name=" + device.getName() + " , address=" + device.getAddress());
            //设备连接成功（SPP）
            //if (listener != null) {
        ...
        if (action.equals(BluetoothDevice.ACTION_ACL_DISCONNECTED)) {
            BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
            BleLog.i( "->onBluetoothDisconnected() - name=" + device.getName() + " , address=" + device.getAddress());
            //设备断开连接(SPP)
            //if (listener != null) {
